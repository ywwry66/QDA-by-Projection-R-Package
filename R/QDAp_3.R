...
drt_3 <- function(mu0,mu1,sigma0,sigma1,sigma,lambda,iter){
  p=length(mu0)
  a=matrix(0,p,iter)
  ...
  a[,1]=d
  #s=nloptr(x0=x0,eval_f=target,eval_g_eq =const,opts = list("algorithm"="NLOPT_GN_ISRES","xtol_rel"=1.0e-4,"maxeval"=10000),lb=rep(-1,p),ub=rep(1,p))
  #print(s)
  #a[,1]=s$solution
  if(iter>1){
      for(j in 2:iter){
...
QDAp_3 <- function(x,y,xnew,lambda=0,iter=1){
  x=data.matrix(x)
  ...
  a=drt(mu0,mu1,sigma0,sigma1,sigma,lambda,iter)
  qda.fit=qda(x%*%a,y)
